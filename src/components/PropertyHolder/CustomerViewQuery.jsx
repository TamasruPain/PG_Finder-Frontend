...
const CustomerViewQuery = () => {
  ...
  useEffect(() => {
    axios.get('https://pg-finder-backend.onrender.com/api/query/' + id)
      .then(result => {
        ...
        return axios.get(`https://pg-finder-backend.onrender.com/api/properties/${queryData.propertyID}`);
      })
      .then(result => {
  ...
  const handleDelete = async (id) => {
    ...
    if (isConfirmed) {
      try {
        const response = await axios.delete(`https://pg-finder-backend.onrender.com/api/query/${id}`);
